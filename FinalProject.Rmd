...
Now I will tidy the data a bit by selecting the key variables of interest then renaming the remaining variables:
...
I first plot changes in pupil diameter across the 3 second study/encoding phase for each word (i.e., bin). As a starting point, see Fig. 4 in [Miller et al. (2019)](https://www.sciencedirect.com/science/article/pii/S0749596X18300846) below:

```{r fig1, out.width = "75%", fig.align='center', echo=FALSE}
include_graphics(img1_path) 
```

I intend to enhance this plot in a number of ways. But, namely, I want to map uncertaintly (previously shown with the standard error bars) via bootstrapping/generatation of outcome draws from a fitted model. For more details on this method, I highly recommend reading up on Claus O. Wilke's [ungeviz package](https://wilkelab.org/ungeviz/articles/posterior-draws.html). Another good reference can be located [here](https://wilkelab.org/ungeviz/reference/bootstrapper.html).

The intended audience for this graph is the **scientific community**, but it is also my hope that interested layspersons may also be able to understand it. The intended message to be communicated is that when instructed to study a list of words, pupil diameter increases throughout the study phase for each word. This increase in pupil dilation is believed to reflect an increase in the amount of attentional effort devoted to a given item.
...
```{r plot1, fig.height=4.5, fig.width=7.5, fig.align='center'}
...
Here I add an academically insprired theme to the plot:
...
```{r plot3_tidy1, fig.align='center', include = FALSE, eval=FALSE}
...
```{r plot3.2_newfacet, fig.align='center', include=FALSE, eval=FALSE}
...
plot4 <- strategy_data %>%
  ...
  ggplot(aes(response, mean_acc, fill = response)) +
      ...
      labs(title = "Recall Accuracy as a Function of Strategy Type and Use of Strategy",
               ...
               x = "",
               caption = "'No' represents individuals who did not report using given strategy, whereas 'Yes' represents those who 
used said strategy. Note that performance is actually better when people report not using a normatively 
ineffective strategy (e.g., rote rehearsal or passive reading) than when they report using these strategies.") +
